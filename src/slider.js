export default class Slider {
    ...
    //一屏滚动完成触发
    _setOnPageShow(){
        let trueLength = this._children.length, page;
        if(this.loop == false) {
        ...
        let target = this.element.children.item(this._currentPage - 1);
        this.onPageShow &&
        typeof this.onPageShow == 'function' &&
        this.onPageShow({page, target});
    }
    ...
    _startAutoSlide(){
        if(
            ...
            //满足以上条件才可以开始自动轮播
            let velocity = 0, displayment, range;
            if(this.autoSlideDirection) {
                displayment = this._pageSize;
                range = [
                    (1 - this._currentPage) * this._pageSize,
                    (2 - this._currentPage) * this._pageSize
                ];
            } else {
                displayment = - this._pageSize;
                range = [
            ...
            let acceleration = this._getAcceleration(velocity, displayment, this.animationDuration);
